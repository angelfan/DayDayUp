module NumberToChineseHelper
  ...
  def number_to_capital_zh(n)
    c_num = ["零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "-", "-", "万", "仟", "佰", "拾", "亿", "仟", "佰", "拾", "万", "仟", "佰", "拾", "元", "角", "分"]
    c_cha = [%w(零元 零拾 零佰 零仟 零万 零亿 亿万 零零零 零零 零万 零亿 亿万 零元), %w(元 零 零 零 万 亿 亿 零 零 万 亿 亿 元)]

    result = ""
    tmp = ("%.0f" % (n.abs.to_f * 100)).to_i
    return '零' if tmp == 0
    raise '整数部分加二位小数长度不能大于15' if tmp.to_s.size > 15
    s_num = tmp.to_s.rjust(15, ' ')

    (0..14).each do |i|
      s_temp = s_num.slice(i, 1)
      next if s_temp == ' '
      result += c_num[s_temp.to_i] + c_num[i + 12];
    end

    (0..12).each do |m|
      result.gsub!(c_cha[0][m], c_cha[1][m])
    end

    if result.index('零分').nil? # 没有分时, 零角改成零
      result.gsub!('零角', '零')
    elsif result.index('零角').nil? # 有没有分有角时, 后面加"整"
      result += '整'
    else
      result.gsub!('零角', '整')
    end
